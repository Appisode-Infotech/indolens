    ...
    try:
        with getConnection().cursor() as cursor:
            get_customer_stats_query = f"""
                                ...
                                FROM DateRange
                                LEFT JOIN customers c
                                ON DATE(c.customer_created_on) = DateRange.date AND c.customer_created_by_store_id = {store_id} 
                                AND c.customer_created_by_store_type = {store_type}
                                GROUP BY DateRange.date"""
    ...
    try:
        with getConnection().cursor() as cursor:
            get_order_stats_query = f"""
                                ...
                                FROM DateRange
                                LEFT JOIN sales_order s
                                ON DATE(s.so_created_on) = DateRange.date AND s.so_created_by_store_type = {store_type} 
                                AND s.so_created_by_store = {store_id}
                                GROUP BY DateRange.date"""
