...
def get_central_inventory_lens():
    try:
        with getConnection().cursor() as cursor:
            get_all_lens_from_central_inventory = f""" SELECT ci.*, creator.admin_name, updater.admin_name,
                                    pc.pc_category_name, pm.pm_material_name, ft.ftype_name, fsh.fshape_name,
                                    co.pcol_color_name, u.unit_name, b.brand_name
                                    FROM central_inventory As ci
                                    LEFT JOIN admin AS creator ON ci.ci_created_by = creator.admin_admin_id
                                    LEFT JOIN admin AS updater ON ci.ci_last_updated_by = updater.admin_admin_id
                                    LEFT JOIN product_categories AS pc ON ci.ci_category_id = pc.pc_category_id
            ...
            get_all_contact_lens_from_central_inventory = f""" SELECT ci.*, creator.admin_name, updater.admin_name,
                                    pc.pc_category_name, pm.pm_material_name, ft.ftype_name, fsh.fshape_name,
                                    co.pcol_color_name, u.unit_name, b.brand_name
                                    FROM central_inventory As ci
                                    LEFT JOIN admin AS creator ON ci.ci_created_by = creator.admin_admin_id
            ...
            return {
                "status": True,
                "lens_list": lens_list,
                "contact_lens_list": contact_lens_list
            }, 200
    except pymysql.Error as e:
        return {"status": False, "message": str(e)}, 301
    except Exception as e:
...
def get_central_inventory_available_lens():
    try:
        with getConnection().cursor() as cursor:
            get_all_lens_from_central_inventory = f""" SELECT ci.*, creator.admin_name, updater.admin_name,
                                    pc.pc_category_name, pm.pm_material_name, ft.ftype_name, fsh.fshape_name,
